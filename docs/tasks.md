...
| M1-T10 | done | db | M1-T03, M1-T09 | Size-tiered compaction scheduler | `aster/db/`, `aster/storage/` | Tier threshold triggers merge; segment count bounded under write soak |
...
| M2-T10 | done | qa | M2-T03, M2-T04 | Recall CI gate (SIFT1M/GloVe subset, recall@10 ≥ 0.95 @ ef=128) | `.github/` or CI + `aster/qa/` | Nightly job; PR gate on regression |
| M2-T11 | done | qa | M2-T04, M2-T08 | Latency bench: 1M×384d search p50 < 5 ms | `aster/qa/`, `docs/bench-latency.md`, `.github/` | CI uploads `--out-json` artifact and `docs/bench-latency.md` documents the exact command + p50 interpretation |
...
| M3-T02 | done | release | M3-T01 | Amalgamated / installable embedded library target | `aster/`, BUILD files | Downstream can depend on one `cc_library` or release tarball |
...
| M7-T01 | done | dist | M7-T00, M0-T04 | Gossip membership + phi-accrual failure detector | `aster/distributed/gossip*` | Nodes join/leave; dead marked within detector window |
...
| ID | Status | Lane | Depends | Title | Touch | Done when |
| --- | --- | --- | --- | --- | --- | --- |
| M8-T01 | done | cloud | M3-T04, M1-T05 | S3 backend complete (multipart, range GET, block cache) | `aster/platform/s3*` | Spot-kill recovery from S3 alone |
